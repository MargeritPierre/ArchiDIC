function [p,t]=distmesh2d_modif(fd,fh,h0,bbox,pfix,varargin)
...
% P.Margerit:
% CONTAINS SOME MINOR MOFICATIONS IN ORDER TO WORK WITH THE PRESENT SCRIPTS
...
count=0;
pold=inf;     
tag = 'distmesh' ; %['distmesh_' num2str(rand(1))] ;   
delete(findobj(gca,'tag',tag)) ;                                    % For first iteration
...
while 1
  ...
  if max(sqrt(sum((p-pold).^2,2))/h0)>ttol
    ...
    pa = findobj(gca,'tag',tag) ;
    if isempty(pa)
        pa = patch('vertices',p,'faces',t,'edgecol','k','facecol',[.8,.9,1],'tag',tag);
    else
        pa.Vertices = p ;
        pa.Faces = t ;
    end
